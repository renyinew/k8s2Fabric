...
JOBSTATUS=$(kubectl get jobs -A|grep createchannel |awk '{print $2}')
while [ "${JOBSTATUS}" != "1/1" ]; do
    echo "Waiting for createchannel job to be completed"
    sleep 1;
    if [ "$(kubectl get pods -A| grep createchannel | awk '{print $3}')" == "Error" ]; then
...
JOBSTATUS=$(kubectl get jobs -A|grep joinchannel |awk '{print $2}')
while [ "${JOBSTATUS}" != "1/1" ]; do
    echo "Waiting for joinchannel job to be completed"
    sleep 1;
    if [ "$(kubectl get pods -A| grep joinchannel | awk '{print $3}')" == "Error" ]; then
...
JOBSTATUS=$(kubectl get jobs -A|grep chaincodeinstall |awk '{print $2}')
while [ "${JOBSTATUS}" != "1/1" ]; do
    echo "Waiting for chaincodeinstall job to be completed"
    sleep 1;
    if [ "$(kubectl get pods -A| grep chaincodeinstall | awk '{print $3}')" == "Error" ]; then
...
JOBSTATUS=$(kubectl get jobs -A|grep chaincodeinstantiate |awk '{print $2}')
while [ "${JOBSTATUS}" != "1/1" ]; do
    echo "Waiting for chaincodeinstantiate job to be completed"
    sleep 1;
    if [ "$(kubectl get pods -A| grep chaincodeinstantiate | awk '{print $3}')" == "Error" ]; then
