...
NUMPENDING=$(kubectl get deployments | grep fabric-zk-kafka | awk '{print $5}' | grep 0 | wc -l | awk '{print $1}')
...
NUMPENDING=$(kubectl get deployments | grep example-com | awk '{print $5}' | grep 0 | wc -l | awk '{print $1}')
while [ "${NUMPENDING}" != "0" ]; do
    echo "Waiting on pending deployments. Deployments pending = ${NUMPENDING}"
    NUMPENDING=$(kubectl get deployments | grep example-com | awk '{print $5}' | grep 0 | wc -l | awk '{print $1}')
    sleep 1
